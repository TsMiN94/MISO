...
public class PrefView extends View {
	...
	public void load() {
		try {
			...
			Scope.getInstace().setHotKeySettingView(hotKeySettingView);
	...
	public void setSelected(PrefBorder settingView) {
		...
		case "hotkey":
			...
			for(int i=0;i<Scope.getInstace().getPrefInfo().getHotkey().size();i++) {
				hotData += TransFormat.getInstance().getVirtualChar(Scope.getInstace().getPrefInfo().getHotkey().get(i));
				if(i<Scope.getInstace().getPrefInfo().getHotkey().size()-1)
						hotData += "+";
			}
			hotkey.setText(hotData);
			...
			String ocrData = TransFormat.getInstance().getVirtualChar(Scope.getInstace().getPrefInfo().getOcrKey());
			
			ocrkey.setText(ocrData);
		//	ocrkey.setText(Scope.getInstace().getPrefInfo().getOcrKey());
